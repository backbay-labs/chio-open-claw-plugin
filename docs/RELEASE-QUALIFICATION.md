...
This lane builds `@chio/openclaw-kernel` from one immutable source commit. A passing workflow
...
- Public repository identity: `backbay-labs/chio-open-claw-plugin`.
- Workflow: `.github/workflows/release.yml`.
- Registry package: `@chio/openclaw-kernel`; GitHub environment: `npm`.
- Release tags: `native/v<package.json version>`, reachable from `main`.
...
frozen acceptance bundle. From `native`:
...
4. Configure the protected `npm` environment before tagging. Restrict deployment
   to `native/v*` tags; use the repository's required release reviewers. A reviewer
...
5. An npm maintainer must register this package's Trusted Publisher with GitHub
   owner `backbay-labs`, repository `chio-open-claw-plugin`, workflow filename `release.yml`,
...
cosign verify-blob --certificate SHA256SUMS.pem --signature SHA256SUMS.sig \
  --certificate-identity 'https://github.com/backbay-labs/chio-open-claw-plugin/.github/workflows/release.yml@refs/tags/native/v0.1.0' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com SHA256SUMS
slsa-verifier verify-artifact package.tgz \
  --provenance-path package.intoto.jsonl \
  --source-uri github.com/backbay-labs/chio-open-claw-plugin --source-tag 'native/v0.1.0'
```
...
Only the `native/` package is delivered by this integration lane. Historical
chat-gateway `@chio/openclaw` and its sibling-checkout Docker build do not prove
native OpenClaw mediation. The `native/v*` tag namespace does not publish those
legacy artifacts. Their release path requires separate qualification.
...
and 25 unit tests passed with zero failed or skipped tests. Staged packing,
...
Job `test` now builds/tests/packs the required `native/` integration. The old
chat-gateway smoke path is not counted as native host coverage. Keeping the
legacy chat gateway itself maintained is outside this integration lane.
