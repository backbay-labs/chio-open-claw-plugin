...
SIGKILL bypassed parent cleanup and left both the agent and relay containers running. The operator validated their exact image/network identities, removed only those recorded containers and their network, and preserved both state/control volumes. This is an unresolved automatic-cleanup limitation, not proof that process lifecycle recovery is complete. Resource mediation remained fenced throughout the recorded observations.
