  ...
  globalThis.fetch=async function(input,init) {
    ...
    if (url === upstream) {
      ...
      if (requests === 1) {
        ...
        assert.equal(outputs.length,spec.calls.length);
...
async function main() {
  ...
  if(mode === "disabled-tools") {
    const observed=calls.map(call=>{
      ...
      const result=returns.find(item=>item.toolCallId===native?.id);
      const text=(result?.content??[]).filter(block=>block.type==="text").map(block=>block.text).join("\n");
      return {name:call.name,nativeCallId:native?.id,returned:!!result,isError:result?.isError,text,
        disabled:!!result&&result.isError===true&&/not found|not available|disabled|blocked/i.test(text)};
    });
    ...
    const passed=!timedOut&&nativeCalls.length===calls.length&&returns.length===calls.length&&observed.every(call=>call.disabled)
      &&fixture.filter(event=>event.phase==="disabled-batch-injected").length===1&&unchanged&&journal.length===0
      &&fileSha(configPath)===configSha&&terminal.confirmedDeliveries===0&&exit.code!==0&&terminal.outcome!=="completed";
    const summary={passed,supplementalOnly:true,liveModel:false,acceptance:false,mode,exit,timedOut,observed,
      ...
      claim:"Unavailable native tools returned errors; independent resource state and dispatch audit were unchanged. OS network and descendant coverage remains separate."};
