...
function installFixture(path) {
  ...
  assert.equal(spec.calls.length,2);
  ...
  globalThis.fetch=async function(input,init) {
    ...
    if (url === upstream) {
      ...
      if (requests === 1) {
        ...
        record({phase:"parallel-batch-injected",calls:items,note:"Two calls intentionally delivered despite unchanged parallel_tool_calls=false"});
      } else {
        assert.equal(outputs.length,2);
        items=[{type:"message",id:"msg_fixture_done",role:"assistant",status:"completed",content:[{type:"output_text",text:"Fixed provider fixture finished. Recorded tool outcomes establish the result.",annotations:[]}]}];
      }
...
async function main() {
  ...
  for(let i=2;i<process.argv.length;i+=2){assert.ok(required.includes(process.argv[i])&&!args[process.argv[i]]&&isAbsolute(process.argv[i+1]??""));args[process.argv[i]]=process.argv[i+1];}
  ...
  const calls=["a","b"].map(letter=>({name:"chio_call",arguments:{tool:"write_file",arguments:{path:`/workspace/openclaw-forced-parallel-${runId}-${letter}.txt`,content:`fixed native parallel fixture ${runId} ${letter}`}}}));
  const spec={schema:"chio.openclaw.parallel-fixture.v1",calls,kernelEndpoint:`http://127.0.0.1:${operator.port}/`,log:join(output,"fixture.jsonl")};
  ...
  assert.ok(calls.every(call=>before.files[basename(call.arguments.arguments.path)]===undefined));
  ...
  const command=[join(pkg,"scripts/protected.mjs"),"--gateway-config",configPath,"--state-dir",runtime,"--image",hostImage,"--model-auth-file",syntheticAuth,
    "--prompt","Use the fixed fixture Chio tool calls. Stop after both returned outcomes. Never retry an uncertain effect."];
  save(join(output,"identity.json"),{claim:"Supplemental deterministic provider batch against actual native OpenClaw and Chio kernel",liveModel:false,acceptance:false,
    ...
    kernelSha256:operator.kernelSha256,resourceImage:operator.image,resourceVolume:operator.volume,auditVolume:operator.auditVolume,
