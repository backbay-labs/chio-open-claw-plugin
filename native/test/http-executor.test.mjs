import {createHttpExecutor} from "../src/http-executor.mjs";
...
for(const fault of ["none","result","stale-request","signature"]){
 test(`guest delivery verification: ${fault}`,async()=>{
  ...
  try{process.env.CHIO_TEST_TRANSPORT_TOKEN="local-test";
   const execute=createHttpExecutor({...binding,transport:"launcher-http-v1",endpoint:"http://127.0.0.1:12345/mcp",tokenEnv:"CHIO_TEST_TRANSPORT_TOKEN",gatewaySessionId:"gateway",allowedTools:["read_text_file"]});
   const request={requestId:"host-call",tool:"read_text_file",arguments:args};
